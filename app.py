...
# MUDANÇA 1: Adicionado suporte a vídeos na galeria
VID_EXTS   = {".mp4", ".mov", ".webm"}
MEDIA_EXTS = IMG_EXTS | VID_EXTS  # imagens + vídeos aceitos na galeria

def listar_galeria(slug):
    """Retorna lista de arquivos de imagem E vídeo na pasta galeria/ do slug."""
    pasta = os.path.join(MODAL_ROOT, slug, "galeria")
    if not os.path.isdir(pasta):
        return []
    arqs = sorted(os.listdir(pasta))
    # MUDANÇA 1: aceita tanto imagens quanto vídeos
    return [f for f in arqs if os.path.splitext(f)[1].lower() in MEDIA_EXTS]
...
def home():
    ...
    try:
        ...
        # ── Modalidades + galeria + múltiplas professoras ──────
        cursor.execute("""
            SELECT
                m.id_modalidade, m.titulo, m.resumo, m.descricao,
                m.nivel, m.slug, m.foto_capa
            FROM modalidades m
            WHERE m.ativo = 1
            ORDER BY m.titulo
        """)
        ...
        for r in cursor.fetchall():
            ...
            # MUDANÇA 1: busca múltiplas professoras da modalidade
            cursor.execute("""
                SELECT c.id_cadastro, c.nome_completo, c.tipo_cadastro,
                       mp.foto_professora
                FROM modalidade_professoras mp
                JOIN cadastro c ON c.id_cadastro = mp.id_cadastro
                WHERE mp.id_modalidade = ?
            ...
            modalidades.append({
                ...
                "foto_capa":        foto_capa,
                "galeria":          gal,
                # compatibilidade: primeiro responsável como principal
                "responsavel_nome": professoras_mod[0]["nome_completo"] if professoras_mod else "",
                "responsavel_tipo": professoras_mod[0]["tipo_cadastro"] if professoras_mod else "",
                "professoras":      professoras_mod,
            })
            galeria_data[r[0]] = [
                f"/static/modalidades/{slug}/galeria/{f}" for f in gal
            ]
 
        # ── Professoras (agora via modalidade_professoras) ─────
        cursor.execute("""
            SELECT DISTINCT
                c.id_cadastro, c.nome_completo, c.tipo_cadastro,
                m.slug, mp.foto_professora,
                m.titulo AS modalidade_nome
            FROM cadastro c
            JOIN modalidade_professoras mp ON mp.id_cadastro = c.id_cadastro
            JOIN modalidades m ON m.id_modalidade = mp.id_modalidade AND m.ativo = 1
    ...
    try:
        ...
        with get_conn() as conn:
            ...
            cursor.execute("""
                ...
                WHERE LOWER(LTRIM(RTRIM(email))) = ? AND cpf = ?
            """, (email, cpf))
            row = cursor.fetchone()
            if not row:
            ...
            aluno_id = row[0]
            cursor.execute("""
                UPDATE cadastro SET senha_hash = ? WHERE id_cadastro = ?
            """, (senha_hash, aluno_id))
            conn.commit()
...
# MUDANÇA 1: vídeos permitidos no upload
ALLOWED_VID = {"mp4", "mov", "webm"}
ALLOWED_MEDIA = ALLOWED_IMG | ALLOWED_VID
...
def admin_page():
    ...
    try:
        ...
        return render_template(
            "admin.html",
            user_name=session.get("user_name", "Administrador"),
            total_usuarios=0, total_modalidades=0,
            total_turmas=0, total_pacotes=0, total_experimentais=0
        )
    finally:
        if cursor: cursor.close()
        if conn:   conn.close()
...
def excluir_usuario(id_cadastro):
    ...
    try:
        ...
        # MUDANÇA 1: remover vínculos de modalidade_professoras
        cursor.execute("""
            DELETE FROM modalidade_professoras WHERE id_cadastro = ?
        """, (id_cadastro,))

        cursor.execute("""
            DELETE mt FROM matriculas mt
            INNER JOIN turmas t ON t.id_turma = mt.id_turma
            INNER JOIN modalidades m ON m.id_modalidade = t.id_modalidade
        ...
        cursor.execute("""
            DELETE pm FROM pacote_modalidades pm
            INNER JOIN modalidades m ON m.id_modalidade = pm.id_modalidade
            WHERE m.id_responsavel_cadastro = ?
        """, (id_cadastro,))

        cursor.execute("""
            DELETE pai FROM primeira_aula_itens pai
            INNER JOIN modalidades m ON m.id_modalidade = pai.id_modalidade
            WHERE m.id_responsavel_cadastro = ?
        """, (id_cadastro,))

        cursor.execute("""
            DELETE wi FROM workshop_inscricoes wi
            INNER JOIN workshops w ON w.id_workshop = wi.id_workshop
            INNER JOIN modalidades m ON m.id_modalidade = w.id_modalidade
            WHERE m.id_responsavel_cadastro = ?
        """, (id_cadastro,))
        ...
        cursor.execute("DELETE FROM workshops WHERE id_instrutor = ?", (id_cadastro,))
        cursor.execute("DELETE FROM modalidades WHERE id_responsavel_cadastro = ?", (id_cadastro,))
        cursor.execute("DELETE FROM cadastro WHERE id_cadastro = ?", (id_cadastro,))

        conn.commit()
        return jsonify(ok=True, mensagem="Usuário e todos os vínculos foram removidos.")
    except pyodbc.Error as e:
        if conn: conn.rollback()
        return jsonify(ok=False, mensagem=str(e)), 500
    finally:
...
def admin_modalidades():
    ...
    try:
        ...
        # MUDANÇA 1: busca foto_capa ao invés de foto_professora
        cursor.execute("""
        ...
        modalidades = []
        for r in cursor.fetchall():
            # MUDANÇA 1: busca professoras vinculadas via modalidade_professoras
            cursor.execute("""
                SELECT mp.id_cadastro, c.nome_completo, mp.foto_professora
                FROM modalidade_professoras mp
                JOIN cadastro c ON c.id_cadastro = mp.id_cadastro
                WHERE mp.id_modalidade = ?
                ORDER BY c.nome_completo
            """, (r[0],))
            profs_mod = [{"id_cadastro":p[0],"nome_completo":p[1],"foto_professora":p[2] or ""}
                         for p in cursor.fetchall()]
            modalidades.append({
                "id_modalidade": r[0], "titulo": r[1], "resumo": r[2] or "",
                "descricao": r[3] or "", "nivel": r[4] or "",
                "slug": r[5] or "", "foto_capa": r[6] or "",
                "professoras": profs_mod,
                # compatibilidade com templates antigos
                "responsavel_nome": profs_mod[0]["nome_completo"] if profs_mod else "",
                "id_responsavel_cadastro": profs_mod[0]["id_cadastro"] if profs_mod else None,
            })
...
def salvar_modalidade():
    """
    MUDANÇA 1:
    - Aceita foto_capa (imagem) OU video_capa (vídeo) para a capa da modalidade
    - Aceita múltiplas professoras via campo 'ids_professoras' (lista de ids separados por vírgula)
    - Cada professora pode ter sua própria foto via 'foto_prof_{id}'
    ...
    # ids_professoras: string separada por vírgula, ex: "3,7,12"
    ids_profs_raw = (request.form.get("ids_professoras") or "").strip()
    ids_professoras = [i.strip() for i in ids_profs_raw.split(",") if i.strip().isdigit()]

    if not titulo:
        return jsonify(ok=False, mensagem="Informe o título."), 400

    slug_val = slugify(titulo)
    pasta_gal  = os.path.join(UPLOAD_ROOT, slug_val, "galeria")
    pasta_capa = os.path.join(UPLOAD_ROOT, slug_val)

    # Capa: aceita imagem ou vídeo
    foto_capa = salvar_arquivo(request.files.get("foto_capa"), pasta_capa, ALLOWED_IMG)
    video_capa = salvar_arquivo(request.files.get("video_capa"), pasta_capa, ALLOWED_VID)
    capa_arquivo = foto_capa or video_capa  # usa o que foi enviado

    # Galeria: imagens e vídeos
    for f in request.files.getlist("galeria"):
        salvar_arquivo(f, pasta_gal, ALLOWED_MEDIA)

    # Fotos individuais de cada professora
    fotos_prof = {}
    for id_p in ids_professoras:
        pasta_prof = os.path.join(UPLOAD_ROOT, slug_val, "professoras", id_p)
        arq = salvar_arquivo(request.files.get(f"foto_prof_{id_p}"), pasta_prof, ALLOWED_IMG)
        if arq:
            fotos_prof[id_p] = arq
    ...
    try:
        ...
        if id_modalidade:
            sql = """UPDATE modalidades
                     SET titulo=?, resumo=?, descricao=?, nivel=?, slug=?"""
            params = [titulo, resumo, descricao, nivel, slug_val]
            if capa_arquivo:
                sql += ", foto_capa=?"
                params.append(capa_arquivo)
            sql += " WHERE id_modalidade=?"
            params.append(id_modalidade)
            cursor.execute(sql, params)

            # MUDANÇA 1: remove todos vínculos anteriores e reinsere
            cursor.execute("DELETE FROM modalidade_professoras WHERE id_modalidade=?", (id_modalidade,))
        else:
            cursor.execute("""
                INSERT INTO modalidades(titulo,resumo,descricao,nivel,slug,foto_capa,ativo)
                OUTPUT INSERTED.id_modalidade
                VALUES(?,?,?,?,?,?,1)
            """, (titulo, resumo, descricao, nivel, slug_val, capa_arquivo))
            row = cursor.fetchone()
            if not row or row[0] is None:
                raise Exception("Não foi possível obter o id_modalidade inserido.")
            id_modalidade = int(row[0])

        # MUDANÇA 1: insere vínculos professora → modalidade
        for id_p in ids_professoras:
            foto_p = fotos_prof.get(id_p)
            cursor.execute("""
                INSERT INTO modalidade_professoras(id_modalidade, id_cadastro, foto_professora)
                VALUES(?,?,?)
            """, (id_modalidade, int(id_p), foto_p))
...
def admin_horarios():
    ...
    try:
        ...
        cursor.execute("""
            SELECT c.id_cadastro, c.nome_completo FROM cadastro c
            WHERE c.tipo_cadastro IN ('professor','administrador')
            ORDER BY c.nome_completo
        """)
        professoras = [{"id_professora":r[0],"nome":r[1]} for r in cursor.fetchall()]

        cursor.execute("""
            SELECT
                t.id_turma, t.nome_exibicao, t.id_modalidade, m.titulo,
                t.id_sala, s.nome, t.id_professora, c.nome_completo,
                h.hora_inicio, h.hora_fim, h.vezes_semana, h.dias_semana,
                t.capacidade_maxima,
                (SELECT COUNT(*) FROM matriculas ma
                 WHERE ma.id_turma = t.id_turma AND ma.ativo = 1) AS alunos_mat
            FROM turmas t
            JOIN modalidades m ON m.id_modalidade = t.id_modalidade
            JOIN salas s ON s.id_sala = t.id_sala
            JOIN horarios h ON h.id_horario = t.id_horario
        ...
        for r in cursor.fetchall():
            turmas.append({
                ...
                "id_modalidade": r[2], "modalidade": r[3],
                "id_sala": r[4], "sala": r[5],
                "id_professora": r[6] or "", "professora": r[7] or "",
                "hora_inicio": str(r[8])[:5] if r[8] else "",
                "hora_fim": str(r[9])[:5] if r[9] else "",
                "vezes_semana": r[10], "dias_semana": r[11] or "",
        ...
        return render_template("admin_horarios.html",
            user_name=session.get("user_name","Administrador"),
            salas=salas, modalidades=modalidades,
            professoras=professoras, turmas=turmas)
    except pyodbc.Error as e:
        flash(str(e),"erro")
        return render_template("admin_horarios.html",
            user_name=session.get("user_name","Administrador"),
            salas=[], modalidades=[], professoras=[], turmas=[])
    finally:
        if cursor: cursor.close()
        if conn:   conn.close()
...
def salvar_turma():
    ...
    try:
        ...
        if id_turma:
            cursor.execute("""
                UPDATE horarios SET hora_inicio=?,hora_fim=?,vezes_semana=?,dias_semana=?
                WHERE id_horario=(SELECT id_horario FROM turmas WHERE id_turma=?)
            """, (hora_inicio, hora_fim, vezes, dias, id_turma))
            cursor.execute("""
                UPDATE turmas SET id_modalidade=?,id_sala=?,id_professora=?,
                       nome_exibicao=?,capacidade_maxima=?
                WHERE id_turma=?
            ...
            cursor.execute("""
                ...
                OUTPUT INSERTED.id_horario VALUES(?,?,?,?)
            ...
            cursor.execute("""
                INSERT INTO turmas(id_modalidade,id_sala,id_professora,id_horario,
                                   nome_exibicao,capacidade_maxima,ativo)
                VALUES(?,?,?,?,?,?,1)
...
def admin_pacotes_page():
    ...
    try:
        ...
        for r in cursor.fetchall():
            ...
            cursor2 = conn.cursor()
            cursor2.execute("""
                SELECT pm.id_modalidade, m.titulo FROM pacote_modalidades pm
                JOIN modalidades m ON m.id_modalidade=pm.id_modalidade
                WHERE pm.id_pacote=? ORDER BY m.titulo
            """, (pid,))
            mods = cursor2.fetchall(); cursor2.close()
            pacotes.append({
                "id_pacote": pid, "nome": r[1], "tipo_cobranca": r[2],
                "valor": float(r[3] or 0), "aulas_por_semana": r[4] or 1,
                "qt_modalidades": r[5] or 1, "ativo": bool(r[6]),
                "observacao": r[7] or "",
                "modalidades_ids": [str(m[0]) for m in mods],
                "modalidades_nomes": [m[1] for m in mods],
            })
        return render_template("admin_pacotes.html",
            user_name=session.get("user_name","Administrador"),
            modalidades=modalidades, pacotes=pacotes)
    except pyodbc.Error as e:
        flash(str(e),"erro")
        return render_template("admin_pacotes.html",
            user_name=session.get("user_name","Administrador"),
...
def salvar_pacote_api():
    ...
    if not nome: return jsonify(ok=False, mensagem="Informe o nome do pacote."), 400
    if qt_mod < 1: return jsonify(ok=False, mensagem="Quantidade de modalidades deve ser >= 1."), 400
    if not mod_ids: return jsonify(ok=False, mensagem="Selecione pelo menos uma modalidade."), 400
    if len(mod_ids) > qt_mod: return jsonify(ok=False, mensagem=f"Selecione no máximo {qt_mod} modalidade(s)."), 400
    ...
    try:
        ...
        if id_pacote:
            cursor.execute("""
                UPDATE pacotes SET nome=?,tipo_cobranca=?,valor=?,aulas_por_semana=?,
                       qt_modalidades=?,ativo=?,observacao=? WHERE id_pacote=?
            """, (nome,tipo,valor,aulas,qt_mod,ativo,obs,id_pacote))
            cursor.execute("DELETE FROM pacote_modalidades WHERE id_pacote=?", (id_pacote,))
        else:
            cursor.execute("""
                INSERT INTO pacotes(nome,tipo_cobranca,valor,aulas_por_semana,qt_modalidades,ativo,observacao)
                OUTPUT INSERTED.id_pacote VALUES(?,?,?,?,?,?,?)
            """, (nome,tipo,valor,aulas,qt_mod,ativo,obs))
            row = cursor.fetchone()
            if not row or row[0] is None: raise Exception("Não foi possível obter o id do pacote.")
            id_pacote = int(row[0])

        for mid in mod_ids:
            cursor.execute("INSERT INTO pacote_modalidades(id_pacote,id_modalidade) VALUES(?,?)",
                           (id_pacote, int(mid)))
        conn.commit()
...
def excluir_pacote_api(id_pacote):
    ...
    try:
        conn = get_conn(); cursor = conn.cursor()
        cursor.execute("DELETE FROM solicitacoes_plano WHERE id_pacote=?", (id_pacote,))
        cursor.execute("DELETE FROM pacote_modalidades WHERE id_pacote=?", (id_pacote,))
        ...
        if "547" in msg or "REFERENCE constraint" in msg:
            return jsonify(ok=False, mensagem="Pacote em uso em outros registros."), 400
    ...
    try:
        ...
        cursor.execute("""
            SELECT ma.id_turma, c.id_cadastro, c.nome_completo, c.email,
                   p.nome AS plano, ma.dias_especificos
            FROM matriculas ma
        ...
        for r in cursor.fetchall():
            ...
            matriculas[t].append({
                "id_cadastro":r[1],"nome_completo":r[2],"email":r[3] or "",
                "plano":r[4] or "",
                # MUDANÇA 3: dias específicos que o aluno escolheu (pode ser NULL = todos)
                "dias_especificos":r[5] or "",
            })
    ...
    try:
        ...
        cursor.execute("""
            SELECT c.id_cadastro, c.nome_completo, c.email,
                   p.nome AS plano, p.aulas_por_semana, ap.id_pacote
            FROM cadastro c
            LEFT JOIN aluno_pacote ap ON ap.id_aluno=c.id_cadastro AND ap.ativo=1
            LEFT JOIN pacotes p ON p.id_pacote=ap.id_pacote
            WHERE c.tipo_cadastro='aluno' ORDER BY c.nome_completo
        """)
...
def matricular_aluno():
    """
    MUDANÇA 3: aceita campo opcional 'dias_especificos' para o admin
    matricular o aluno em apenas alguns dias da turma.
    Ex: turma ocorre SEG,QUA,SEX — admin pode matricular só SEG,QUA.
    """
    data = request.get_json(silent=True) or {}
    id_turma = data.get("id_turma")
    id_aluno = data.get("id_aluno")
    # MUDANÇA 3: dias específicos (opcional)
    dias_especificos = (data.get("dias_especificos") or "").strip() or None
    ...
    try:
        ...
        """, (id_turma,))
        row = cursor.fetchone()
        if not row: return jsonify(ok=False, mensagem="Turma não encontrada."), 404

        cap, ocup, id_modal = row[0] or 0, row[1] or 0, row[2]
        if cap > 0 and ocup >= cap:
            return jsonify(ok=False, codigo="TURMA_CHEIA",
                mensagem=f"Turma lotada. Capacidade máxima: {cap} alunos."), 400

        cursor.execute("""
            SELECT ap.id_pacote, p.nome, p.aulas_por_semana FROM aluno_pacote ap
            JOIN pacotes p ON p.id_pacote=ap.id_pacote
            WHERE ap.id_aluno=? AND ap.ativo=1
        """, (id_aluno,))
        pacote_row = cursor.fetchone()
        if not pacote_row:
            return jsonify(ok=False, codigo="SEM_PLANO",
                mensagem="O aluno não possui um pacote de aulas ativo."), 400

        id_pacote, nome_plano, aulas_semanais = pacote_row[0], pacote_row[1], pacote_row[2] or 0

        cursor.execute("SELECT pm.id_modalidade FROM pacote_modalidades pm WHERE pm.id_pacote=?", (id_pacote,))
        mods_permitidas = {r[0] for r in cursor.fetchall()}
        if mods_permitidas and int(id_modal) not in mods_permitidas:
            return jsonify(ok=False, codigo="PLANO_INCOMPATIVEL",
                mensagem=f"O plano \"{nome_plano}\" não inclui esta modalidade."), 400
        ...
        if aulas_semanais > 0:
            cursor.execute("""
                ...
                JOIN horarios h ON h.id_horario=t.id_horario
                WHERE ma.id_aluno=? AND ma.ativo=1 AND h.vezes_semana>=1
            """, (id_aluno,))
            qtd_atual = cursor.fetchone()[0] or 0
            if qtd_atual >= aulas_semanais:
                return jsonify(ok=False, codigo="LIMITE_AULAS",
                    mensagem=f"Aluno já atingiu o limite de {aulas_semanais} aula(s)/semana do plano \"{nome_plano}\"."), 400

        cursor.execute("SELECT id_matricula, ativo FROM matriculas WHERE id_turma=? AND id_aluno=?",
                       (id_turma, id_aluno))
        exist = cursor.fetchone()
        if exist:
            if exist[1]: return jsonify(ok=False, mensagem="Aluno já matriculado nesta turma."), 400
            cursor.execute("UPDATE matriculas SET ativo=1, dias_especificos=? WHERE id_matricula=?",
                           (dias_especificos, exist[0]))
        else:
            cursor.execute("""
                INSERT INTO matriculas(id_turma, id_aluno, ativo, dias_especificos)
                VALUES(?,?,1,?)
            """, (id_turma, id_aluno, dias_especificos))
...
def desmatricular_aluno():
    ...
    try:
        conn = get_conn(); cursor = conn.cursor()
        cursor.execute("UPDATE matriculas SET ativo=0 WHERE id_turma=? AND id_aluno=? AND ativo=1",
                       (id_turma, id_aluno))
        if cursor.rowcount == 0: return jsonify(ok=False, mensagem="Matrícula não encontrada."), 404
        conn.commit()
...
def contato():
    ...
    print({"tipo":"contato_geral","nome":nome,"telefone":telefone,
           "email":email,"modalidade":modalidade,"mensagem":mensagem})
...
def registrar_aluno():
    ...
    if len(cpf) != 11 or not cpf.isdigit():
        return jsonify(ok=False, mensagem="CPF inválido."), 400
    if not senha_forte_valida(senha):
        return jsonify(ok=False, mensagem="A senha deve ter no mínimo 8 caracteres, com letra maiúscula, minúscula, número e símbolo."), 400
    ...
    try:
        conn = get_conn(); cursor = conn.cursor()
        cursor.execute("SELECT 1 FROM cadastro WHERE email=?", (email,))
        if cursor.fetchone(): return jsonify(ok=False, mensagem="Já existe um cadastro com este e-mail."), 400
        cursor.execute("SELECT 1 FROM cadastro WHERE cpf=?", (cpf,))
        if cursor.fetchone(): return jsonify(ok=False, mensagem="Já existe um cadastro com este CPF."), 400
        cursor.execute("""
            INSERT INTO cadastro(nome_completo,email,cpf,senha_hash,termo_imagem,termo_seguranca,tipo_cadastro)
            VALUES(?,?,?,?,?,?,'aluno')
        """, (nome,email,cpf,senha_hash,termo_imagem,termo_seguranca))
        conn.commit()
...
def login_post():
    ...
    try:
        ...
        """, (email,))
        row = cursor.fetchone()
        if not row: return jsonify(ok=False, mensagem="Usuário não encontrado."), 400
        ...
        destinos = {
            "aluno": "aluno_page", "administrador": "admin_page",
            "professor": "professor_page"
        }
        destino = url_for(destinos.get(tipo_cadastro, "login_page"))
        return jsonify(ok=True, mensagem="Login realizado com sucesso.",
                       redirect_url=destino, tipo_cadastro=tipo_cadastro), 200
    except pyodbc.Error as e:
...
def admin_primeira_aula():
    ...
    try:
        conn = get_conn(); cursor = conn.cursor()
        cursor.execute("""
            SELECT m.id_modalidade, m.titulo, m.nivel,
                   (SELECT COUNT(*) FROM primeira_aula_itens i
                    WHERE i.id_modalidade=m.id_modalidade AND i.ativo=1) AS total_itens
            FROM modalidades m WHERE m.ativo=1 ORDER BY m.titulo
        """)
        modalidades = [{"id_modalidade":r[0],"titulo":r[1],"nivel":r[2] or "","total_itens":r[3] or 0}
                       for r in cursor.fetchall()]
        return render_template("admin_primeira_aula.html",
            user_name=session.get("user_name","Administrador"), modalidades=modalidades)
    except pyodbc.Error:
        return render_template("admin_primeira_aula.html",
            user_name=session.get("user_name","Administrador"), modalidades=[])
    finally:
        if cursor: cursor.close()
        if conn:   conn.close()
...
def listar_itens_primeira_aula(id_modalidade):
    ...
    try:
        conn = get_conn(); cursor = conn.cursor()
        cursor.execute("""
            SELECT id_item,id_modalidade,nome,categoria,funcao,observacao,obrigatorio,ordem
            FROM primeira_aula_itens
            WHERE id_modalidade=? AND ativo=1
            ORDER BY CASE WHEN ordem IS NULL THEN 1 ELSE 0 END, ordem, nome
        """, (id_modalidade,))
        itens = [{"id_item":r[0],"id_modalidade":r[1],"nome":r[2] or "",
                  "categoria":r[3] or "outros","funcao":r[4] or "","observacao":r[5] or "",
                  "obrigatorio":bool(r[6]),"ordem":r[7]} for r in cursor.fetchall()]
...
def salvar_item_primeira_aula():
    ...
    ordem = data.get("ordem") or None

    cats_validas = {"vestuario","equipamento","higiene","alimentacao","documentos","outros"}
    if categoria not in cats_validas: categoria = "outros"
    if not nome: return jsonify(ok=False, mensagem="Informe o nome do item."), 400
    if not id_modal: return jsonify(ok=False, mensagem="Informe a modalidade."), 400

    conn = cursor = None
    try:
        conn = get_conn(); cursor = conn.cursor()
        cursor.execute("SELECT 1 FROM modalidades WHERE id_modalidade=? AND ativo=1", (id_modal,))
        if not cursor.fetchone(): return jsonify(ok=False, mensagem="Modalidade não encontrada."), 404

        if id_item:
            cursor.execute("""
                UPDATE primeira_aula_itens
                SET id_modalidade=?,nome=?,categoria=?,funcao=?,observacao=?,obrigatorio=?,ordem=?
                WHERE id_item=?
            """, (id_modal,nome,categoria,funcao,observacao,obrigatorio,ordem,id_item))
        else:
            cursor.execute("""
                INSERT INTO primeira_aula_itens(id_modalidade,nome,categoria,funcao,observacao,obrigatorio,ordem,ativo)
                VALUES(?,?,?,?,?,?,?,1)
            """, (id_modal,nome,categoria,funcao,observacao,obrigatorio,ordem))
...
def excluir_item_primeira_aula(id_item):
    ...
    try:
        conn = get_conn(); cursor = conn.cursor()
        cursor.execute("UPDATE primeira_aula_itens SET ativo=0 WHERE id_item=?", (id_item,))
        if cursor.rowcount == 0: return jsonify(ok=False, mensagem="Item não encontrado."), 404
        conn.commit()
    ...
    try:
        ...
        cursor.execute("""
            SELECT id_workshop,nome,id_modalidade,id_instrutor,descricao,requisitos,
                   observacao,data_workshop,hora_inicio,hora_fim,id_sala,vagas_totais,valor,status,imagem_capa
        ...
        return jsonify(ok=True, workshop={
            "id_workshop":r[0],"nome":r[1],"id_modalidade":r[2],"id_instrutor":r[3],
            "descricao":r[4] or "","requisitos":r[5] or "","observacao":r[6] or "",
            "data_workshop":r[7].isoformat() if r[7] else None,
            "hora_inicio":str(r[8])[:5] if r[8] else "","hora_fim":str(r[9])[:5] if r[9] else "",
...
def listar_inscricoes_workshop(id_ws):
    ...
    try:
        conn = get_conn(); cursor = conn.cursor()
        cursor.execute("SELECT nome,data_workshop,hora_inicio,hora_fim,vagas_totais,status FROM workshops WHERE id_workshop=?", (id_ws,))
        r = cursor.fetchone()
        if not r: return jsonify(ok=False, mensagem="Workshop não encontrado."), 404
        workshop = {"id_workshop":id_ws,"nome":r[0],
            "data_workshop":r[1].isoformat() if r[1] else None,
            "hora_inicio":str(r[2])[:5] if r[2] else "","hora_fim":str(r[3])[:5] if r[3] else "",
            "vagas_totais":r[4] or 0,"status":r[5] or "rascunho"}
        cursor.execute("""
            SELECT c.id_cadastro, c.nome_completo, c.email, wi.data_inscricao
            FROM workshop_inscricoes wi JOIN cadastro c ON c.id_cadastro=wi.id_participante
            WHERE wi.id_workshop=? AND wi.ativo=1 ORDER BY wi.data_inscricao, c.nome_completo
        """, (id_ws,))
        inscritos = [{"id_cadastro":r[0],"nome_completo":r[1],"email":r[2] or "",
                      "data_inscricao":r[3].isoformat() if r[3] else None}
                     for r in cursor.fetchall()]
...
def inscrever_workshop():
    ...
    if not id_ws or not id_part: return jsonify(ok=False, mensagem="Informe id_workshop e id_participante."), 400
    ...
    try:
        ...
        cursor.execute("""
            SELECT vagas_totais, status,
                   (SELECT COUNT(*) FROM workshop_inscricoes wi WHERE wi.id_workshop=w.id_workshop AND wi.ativo=1)
        ...
        if status == "encerrado":
            return jsonify(ok=False, codigo="WS_ENCERRADO", mensagem="Inscrições encerradas."), 400
        if vagas > 0 and inscritos >= vagas:
            return jsonify(ok=False, codigo="WS_CHEIO", mensagem=f"Workshop lotado ({vagas} vagas)."), 400
        cursor.execute("SELECT id_inscricao, ativo FROM workshop_inscricoes WHERE id_workshop=? AND id_participante=?",
                       (id_ws, id_part))
        exist = cursor.fetchone()
        if exist:
            if exist[1]: return jsonify(ok=False, mensagem="Participante já inscrito."), 400
            cursor.execute("UPDATE workshop_inscricoes SET ativo=1 WHERE id_inscricao=?", (exist[0],))
        else:
            cursor.execute("INSERT INTO workshop_inscricoes(id_workshop,id_participante,ativo) VALUES(?,?,1)",
                           (id_ws, id_part))
        conn.commit()
...
def desinscrever_workshop():
    ...
    try:
        ...
        """, (id_ws, id_part))
        if cursor.rowcount == 0: return jsonify(ok=False, mensagem="Inscrição não encontrada."), 404
        conn.commit()
...
def fim_do_mes(d=None):
    d = d or date.today()
    if d.month == 12: return d.replace(day=31)
...
def professor_page():
    return render_template("professor.html",
        user_name=session.get("user_name","Professora"), user_tipo="professor")
...
def professor_dados():
    ...
    try:
        ...
        if has_id_prof_col:
            # MUDANÇA 1: inclui turmas via modalidade_professoras também
            cursor.execute("""
                ...
                WHERE t.ativo=1
                  AND (
                    t.id_professora = ?
                    OR EXISTS (
                        SELECT 1 FROM modalidade_professoras mp
                        WHERE mp.id_modalidade = t.id_modalidade
                          AND mp.id_cadastro = ?
                    )
                  )
                ORDER BY m.titulo, h.hora_inicio
            ...
            cursor.execute("""
                ...
                JOIN salas s ON s.id_sala=t.id_sala
                JOIN horarios h ON h.id_horario=t.id_horario
                WHERE t.ativo=1 AND EXISTS (
                    SELECT 1 FROM modalidade_professoras mp
                    WHERE mp.id_modalidade=t.id_modalidade AND mp.id_cadastro=?
                )
                ORDER BY m.titulo, h.hora_inicio
            """, (id_prof,))
        ...
        for r in cursor.fetchall():
            turma = {
                "id_turma":r[0],"nome_exibicao":r[1] or "","capacidade_maxima":r[2] or 0,
                "modalidade":r[3],"sala":r[4],
                "hora_inicio":str(r[5])[:5] if r[5] else "",
                "hora_fim":str(r[6])[:5] if r[6] else "",
                "dias_semana":r[7] or "","alunos":[],
            }
            cursor.execute("""
                SELECT c.id_cadastro, c.nome_completo, c.email, c.termo_imagem, p.nome AS plano
                FROM matriculas ma
                JOIN cadastro c ON c.id_cadastro=ma.id_aluno
                LEFT JOIN aluno_pacote ap ON ap.id_aluno=c.id_cadastro AND ap.ativo=1
                LEFT JOIN pacotes p ON p.id_pacote=ap.id_pacote
                WHERE ma.id_turma=? AND ma.ativo=1 ORDER BY c.nome_completo
            """, (turma["id_turma"],))
            for a in cursor.fetchall():
                turma["alunos"].append({
                    "id_cadastro":a[0],"nome_completo":a[1],"email":a[2] or "",
                    "termo_imagem":bool(a[3]),"plano":a[4] or "",
                })
            turmas.append(turma)
        ...
        if _tabela_existe(cursor,"reposicoes") and _tabela_existe(cursor,"aulas_canceladas"):
            try:
                cursor.execute("""
                    SELECT rep.id_reposicao, c.nome_completo, rep.tipo, rep.usada,
                           t.nome_exibicao, m.titulo, ac.data_aula, ac.motivo
                    FROM reposicoes rep
                ...
                """, (id_prof,))
                for r in cursor.fetchall():
                    reposicoes.append({"id_reposicao":r[0],"aluno_nome":r[1],"tipo":r[2],
                        "usada":bool(r[3]),"turma_nome":r[4] or r[5],
                        "data_aula":str(r[6]) if r[6] else "","motivo":r[7] or ""})
            except pyodbc.Error:
                reposicoes = []
        ...
        if _tabela_existe(cursor,"aulas_canceladas"):
            try:
                cursor.execute("""
                    SELECT ac.id_cancelamento, m.titulo, t.nome_exibicao,
                    ...
                    JOIN turmas t ON t.id_turma=ac.id_turma
                    JOIN modalidades m ON m.id_modalidade=t.id_modalidade
                    WHERE ac.cancelado_por=? ORDER BY ac.cancelado_em DESC
                """, (id_prof,))
                for r in cursor.fetchall():
                    historico.append({"id_cancelamento":r[0],"turma_nome":r[2] or r[1],
                        "data_aula":str(r[3]) if r[3] else "","motivo":r[4] or "",
                        "cancelado_em":str(r[5])[:10] if r[5] else "","alunos_afetados":r[6] or 0})
            except pyodbc.Error:
                historico = []
...
def professor_cancelar_aula():
    ...
    try:
        ...
        if not _tabela_existe(cursor,"aulas_canceladas"):
            return jsonify(ok=False, mensagem="Funcionalidade de cancelamento ainda não configurada no banco."), 400
        cursor.execute("SELECT 1 FROM aulas_canceladas WHERE id_turma=? AND data_aula=?", (id_turma, data_aula))
        if cursor.fetchone(): return jsonify(ok=False, mensagem="Esta aula já foi cancelada anteriormente."), 400
        cursor.execute("INSERT INTO aulas_canceladas(id_turma,data_aula,motivo,cancelado_por) VALUES(?,?,?,?)",
        ...
        if _tabela_existe(cursor,"reposicoes"):
            ...
            for id_aluno in alunos:
                cursor.execute("""
                    INSERT INTO reposicoes(id_aluno,id_cancelamento,tipo,usada,data_aula_orig,id_turma_orig,valida_ate)
                    VALUES(?,?,'especial',0,?,?,?)
...
def aluno_page():
    return render_template("aluno.html",
        user_name=session.get("user_name","Aluno"), user_tipo="aluno")
...
def aluno_dados():
    ...
    try:
        ...
        """, (id_aluno,))
        row = cursor.fetchone()
        if not row: return jsonify(ok=False, mensagem="Aluno não encontrado."), 404
        ...
        if tem_aluno_pacote:
            ...
            if has_status_col:
                cursor.execute("""
                    ...
                    FROM aluno_pacote ap JOIN pacotes p ON p.id_pacote=ap.id_pacote
                    WHERE ap.id_aluno=? AND ap.ativo=1
                """, (id_aluno,))
            else:
                cursor.execute("""
            ...
            if p:
                ...
                plano = {
                    ...
                    "valor":float(p[3] or 0),"aulas_por_semana":p[4] or 1,
                    "qt_modalidades":p[5] or 1,"status":status_acesso,
                    "data_inicio":str(p[6]) if p[6] else "","data_fim":str(p[7]) if p[7] else "",
                    "modalidades_nomes":[r[1] for r in mods_rows],
                    "modalidades_ids":list(mods_plano_ids),
                }
 
        # ── Turmas matriculadas ─────────────────────────────────
        # MUDANÇA 3: inclui dias_especificos na query
        cursor.execute("""
            SELECT
                t.id_turma, t.nome_exibicao, t.capacidade_maxima,
                m.titulo, s.nome, h.hora_inicio, h.hora_fim, h.dias_semana,
                c.nome_completo, ma.dias_especificos
            FROM matriculas ma
            JOIN turmas t ON t.id_turma=ma.id_turma
            JOIN modalidades m ON m.id_modalidade=t.id_modalidade
            JOIN salas s ON s.id_sala=t.id_sala
        ...
        ids_mat = set()
        for r in cursor.fetchall():
            # dias efetivos: os específicos escolhidos, ou todos os dias da turma
            dias_efetivos = r[9] if r[9] else r[7] or ""
            turmas_mat.append({
                "id_turma":r[0],"nome_exibicao":r[1] or "","capacidade_maxima":r[2] or 0,
                "modalidade":r[3],"sala":r[4],
                "hora_inicio":str(r[5])[:5] if r[5] else "",
                "hora_fim":str(r[6])[:5] if r[6] else "",
                "dias_semana":r[7] or "",
                "dias_especificos":r[9] or "",  # MUDANÇA 3
                "dias_efetivos":dias_efetivos,   # MUDANÇA 3: dias reais do aluno
                "professora":r[8] or "","tipo_matricula":"normal","inscritos":0,
            })
            ids_mat.add(r[0])
        ...
        cursor.execute("""
            SELECT
                t.id_turma, t.nome_exibicao, t.capacidade_maxima,
                m.id_modalidade, m.titulo, s.nome,
                h.hora_inicio, h.hora_fim, h.dias_semana,
                c.nome_completo,
        ...
        turmas_disp = []
        for r in cursor.fetchall():
            id_t = r[0]; id_mod = r[3]; cap = r[2] or 0; ocp = r[10] or 0
            ...
            if not no_plano:
                if not lotada:
                    turmas_disp.append({
                        ...
                        "hora_inicio":str(r[6])[:5] if r[6] else "","hora_fim":str(r[7])[:5] if r[7] else "",
                        "dias_semana":r[8] or "","professora":r[9] or "","inscritos":ocp,
            ...
            turmas_disp.append({
                ...
                "hora_inicio":str(r[6])[:5] if r[6] else "","hora_fim":str(r[7])[:5] if r[7] else "",
                "dias_semana":r[8] or "","professora":r[9] or "","inscritos":ocp,
                "lotada":lotada,"apenas_reposicao_especial":False,
            })
 
        # ── Reposições ─────────────────────────────────────────
        # MUDANÇA 2: turmas disponíveis para reposição = qualquer turma
        #            de modalidade que tenha UMA DAS professoras do aluno
        reposicoes = []
        turmas_reposicao = []  # MUDANÇA 2: novo campo
        if _tabela_existe(cursor,"reposicoes"):
            try:
                cursor.execute("""
                    ...
                    WHERE rep.id_aluno=? ORDER BY rep.criado_em DESC
                """, (id_aluno,))
                for r in cursor.fetchall():
                    reposicoes.append({
                        "id_reposicao":r[0],"tipo":r[1],"usada":bool(r[2]),
            ...
            # MUDANÇA 2: busca turmas disponíveis para reposição
            # São todas as turmas de qualquer modalidade que tenha
            # pelo menos uma professora em comum com o aluno.
            try:
                cursor.execute("""
                    SELECT DISTINCT
                        t.id_turma, t.nome_exibicao, t.capacidade_maxima,
                        m.titulo AS modalidade, s.nome AS sala,
                        h.hora_inicio, h.hora_fim, h.dias_semana,
                        (SELECT COUNT(*) FROM matriculas ma2 WHERE ma2.id_turma=t.id_turma AND ma2.ativo=1) AS inscritos
                    FROM turmas t
                    JOIN modalidades m ON m.id_modalidade=t.id_modalidade
                    JOIN salas s ON s.id_sala=t.id_sala
                    JOIN horarios h ON h.id_horario=t.id_horario
                    WHERE t.ativo=1
                      AND m.id_modalidade IN (
                          -- modalidades das turmas em que o aluno está matriculado
                          SELECT DISTINCT t2.id_modalidade
                          FROM matriculas ma3
                          JOIN turmas t2 ON t2.id_turma=ma3.id_turma
                          WHERE ma3.id_aluno=? AND ma3.ativo=1
                      )
                    ORDER BY m.titulo, h.hora_inicio
                """, (id_aluno,))
                for r in cursor.fetchall():
                    cap = r[2] or 0; ocp = r[8] or 0
                    turmas_reposicao.append({
                        "id_turma":r[0],"nome_exibicao":r[1] or "","capacidade_maxima":cap,
                        "modalidade":r[3],"sala":r[4],
                        "hora_inicio":str(r[5])[:5] if r[5] else "",
                        "hora_fim":str(r[6])[:5] if r[6] else "",
                        "dias_semana":r[7] or "","inscritos":ocp,
                        "lotada":cap > 0 and ocp >= cap,
                    })
            except pyodbc.Error:
                turmas_reposicao = []
        ...
        # ── Pacotes disponíveis ────────────────────────────────
        cursor.execute("""
            SELECT p.id_pacote, p.nome, p.tipo_cobranca, p.valor, p.aulas_por_semana, p.qt_modalidades
            FROM pacotes p WHERE p.ativo=1 ORDER BY p.valor
        """)
        pacotes = []
        for r in cursor.fetchall():
            ...
            cursor.execute("""
                SELECT m.titulo FROM pacote_modalidades pm
                JOIN modalidades m ON m.id_modalidade=pm.id_modalidade WHERE pm.id_pacote=?
            """, (pid,))
            mods_p = [x[0] for x in cursor.fetchall()]
            pacotes.append({"id_pacote":pid,"nome":r[1],"tipo_cobranca":r[2],
                "valor":float(r[3] or 0),"aulas_por_semana":r[4] or 1,"qt_modalidades":r[5] or 1,
                "status":"ativo","modalidades_nomes":mods_p})
        ...
        if _tabela_existe(cursor,"solicitacoes_plano"):
            try:
                cursor.execute("""
                    ...
                    FROM solicitacoes_plano sp JOIN pacotes p ON p.id_pacote=sp.id_pacote
                    WHERE sp.id_aluno=? AND sp.status='pendente'
                """, (id_aluno,))
                r_sol = cursor.fetchone()
                if r_sol:
                    sol_pend = {"id_solicitacao":r_sol[0],"id_pacote":r_sol[1],"pacote_nome":r_sol[2]}
            except pyodbc.Error:
                sol_pend = None
        ...
        if _tabela_existe(cursor,"historico_pagamentos"):
            try:
                cursor.execute("""
                    SELECT data_pagamento, valor, descricao FROM historico_pagamentos
                    WHERE id_aluno=? ORDER BY data_pagamento DESC
                """, (id_aluno,))
                hist_pag = [{"data_pagamento":str(r[0]) if r[0] else "","valor":float(r[1] or 0),"descricao":r[2] or ""}
                            for r in cursor.fetchall()]
            except pyodbc.Error:
                hist_pag = []
 
        return jsonify(
            ok=True, aluno=aluno, plano=plano,
            turmas_matriculadas=turmas_mat,
            turmas_disponiveis=turmas_disp,
            reposicoes=reposicoes,
            turmas_reposicao=turmas_reposicao,  # MUDANÇA 2: novo campo na resposta
            pacotes=pacotes, status_acesso=status_acesso,
...
def aluno_inscrever():
    """
    MUDANÇA 3: aceita campo opcional 'dias_especificos' para o aluno
    escolher em quais dias da turma quer se inscrever.
    """
    data_req = request.get_json(silent=True) or {}
    id_turma = data_req.get("id_turma")
    id_aluno = session["user_id"]
    # MUDANÇA 3: dias específicos escolhidos pelo aluno (opcional)
    dias_especificos = (data_req.get("dias_especificos") or "").strip() or None

    if not id_turma: return jsonify(ok=False, mensagem="Informe a turma."), 400
    ...
    try:
        ...
        if _tabela_existe(cursor,"aluno_pacote") and _col_exists(cursor,"aluno_pacote","status_acesso"):
            ...
            if r_st and r_st[0] == "suspenso":
                return jsonify(ok=False, mensagem="Seu acesso está suspenso por falta de pagamento."), 403
        ...
        if _tabela_existe(cursor,"aluno_pacote"):
            ...
            cursor.execute("""
                SELECT pm.id_modalidade FROM turmas t
        ...
        # Conflito de horário (MUDANÇA 3: considera dias_especificos se informado)
        cursor.execute("""
        ...
        if nova:
            # Dias efetivos da nova matrícula
            dias_nova_str = dias_especificos if dias_especificos else (nova[0] or "")
            dias_nova = set(d.strip().lower() for d in dias_nova_str.split(",") if d.strip())

            cursor.execute("""
                SELECT h.dias_semana, h.hora_inicio, h.hora_fim, ma.dias_especificos
                FROM matriculas ma JOIN turmas t ON t.id_turma=ma.id_turma
                JOIN horarios h ON h.id_horario=t.id_horario
                WHERE ma.id_aluno=? AND ma.ativo=1
            """, (id_aluno,))
            for ex in cursor.fetchall():
                # Dias efetivos da matrícula existente
                dias_ex_str = ex[3] if ex[3] else (ex[0] or "")
                dias_ex = set(d.strip().lower() for d in dias_ex_str.split(",") if d.strip())
                if dias_nova & dias_ex:
        ...
        if ex:
            ...
            cursor.execute("UPDATE matriculas SET ativo=1, dias_especificos=? WHERE id_turma=? AND id_aluno=?",
                           (dias_especificos, id_turma, id_aluno))
        else:
            cursor.execute("INSERT INTO matriculas(id_turma,id_aluno,ativo,dias_especificos) VALUES(?,?,1,?)",
                           (id_turma, id_aluno, dias_especificos))
...
def aluno_cancelar_aula():
    ...
    try:
        ...
        if not cursor.fetchone(): return jsonify(ok=False, mensagem="Você não está inscrita nesta turma."), 400
        cursor.execute("""
            SELECT 1 FROM reposicoes WHERE id_aluno=? AND id_turma_orig=? AND data_aula_orig=? AND tipo='normal'
        """, (id_aluno, id_turma, data_aula))
        if cursor.fetchone(): return jsonify(ok=False, mensagem="Você já cancelou esta aula."), 400
        valida_ate = fim_do_mes()
        cursor.execute("""
...
def aluno_usar_reposicao_especial():
    """
    MUDANÇA 2: agora aceita qualquer turma de modalidade que tenha
    uma professora em comum com as turmas do aluno — não só a modalidade original.
    """
    data_req     = request.get_json(silent=True) or {}
    id_turma     = data_req.get("id_turma")
    ...
    try:
        ...
        rep = cursor.fetchone()
        if not rep: return jsonify(ok=False, mensagem="Reposição não encontrada."), 404
        if rep[0]: return jsonify(ok=False, mensagem="Esta reposição já foi utilizada."), 400
        if rep[1] and date.today() > rep[1]: return jsonify(ok=False, mensagem="Esta reposição expirou."), 400

        # MUDANÇA 2: valida que a turma alvo pertence a uma modalidade
        # em que o aluno já esteve/está matriculado (mesma modalidade da turma original)
        cursor.execute("""
            SELECT t.id_modalidade FROM turmas t WHERE t.id_turma=?
        """, (id_turma,))
        row_turma = cursor.fetchone()
        if not row_turma: return jsonify(ok=False, mensagem="Turma não encontrada."), 404
        id_modal_alvo = row_turma[0]

        # Modalidades das turmas em que o aluno está ou esteve matriculado
        cursor.execute("""
            SELECT DISTINCT t2.id_modalidade FROM matriculas ma
            JOIN turmas t2 ON t2.id_turma=ma.id_turma
            WHERE ma.id_aluno=?
        """, (id_aluno,))
        mods_do_aluno = {r[0] for r in cursor.fetchall()}

        if id_modal_alvo not in mods_do_aluno:
            return jsonify(ok=False,
                mensagem="Você só pode repor em turmas de modalidades em que já está ou esteve matriculada."), 400
        ...
        if nova:
            ...
            """, (id_aluno,))
            for ex in cursor.fetchall():
                dias_nova = set((nova[0] or "").lower().replace(" ","").split(","))
                dias_ex_str = ex[3] if ex[3] else (ex[0] or "")
                dias_ex = set(dias_ex_str.lower().replace(" ","").split(","))
                if dias_nova & dias_ex:
                    ini_n=str(nova[1])[:5]; fim_n=str(nova[2])[:5]
                    ini_e=str(ex[1])[:5]; fim_e=str(ex[2])[:5]
                    if ini_n < fim_e and fim_n > ini_e:
        ...
        cursor.execute("SELECT ativo FROM matriculas WHERE id_turma=? AND id_aluno=?", (id_turma, id_aluno))
        ex = cursor.fetchone()
        if ex and ex[0]: return jsonify(ok=False, mensagem="Você já está inscrita nesta turma."), 400
...
def aluno_solicitar_plano():
    ...
    id_aluno  = session["user_id"]
    if not id_pacote: return jsonify(ok=False, mensagem="Informe o pacote."), 400

    conn = cursor = None
    try:
        conn = get_conn(); cursor = conn.cursor()
        if not _tabela_existe(cursor,"solicitacoes_plano"):
            return jsonify(ok=False, mensagem="Funcionalidade ainda não configurada."), 400
        cursor.execute("SELECT 1 FROM solicitacoes_plano WHERE id_aluno=? AND status='pendente'", (id_aluno,))
        if cursor.fetchone():
            return jsonify(ok=False, mensagem="Você já tem uma solicitação pendente."), 400
        tem_plano = False
        if _tabela_existe(cursor,"aluno_pacote"):
        ...
        if has_tipo_col:
            cursor.execute("INSERT INTO solicitacoes_plano(id_aluno,id_pacote,tipo_solicitacao,status) VALUES(?,?,?,'pendente')",
                           (id_aluno, id_pacote, tipo))
        else:
            cursor.execute("INSERT INTO solicitacoes_plano(id_aluno,id_pacote,status) VALUES(?,?,'pendente')",
                           (id_aluno, id_pacote))
        conn.commit()
...
def proximo_vencimento(data_inicio, tipo_cobranca, ultimo_pagamento=None):
    ...
    if tipo_cobranca == 'mensal': return base + relativedelta(months=1)
...
def pacotes_controle_dados():
    ...
    try:
        ...
        cursor.execute("""
            SELECT sp.id_solicitacao, sp.tipo_solicitacao, sp.status, sp.criado_em, sp.obs_admin,
                   c.nome_completo, c.email, p.id_pacote, p.nome, p.tipo_cobranca, p.valor,
            ...
            ORDER BY CASE sp.status WHEN 'pendente' THEN 0 ELSE 1 END, sp.criado_em DESC
        """)
        solicitacoes = []
        for r in cursor.fetchall():
            sid = r[0]
            ...
            solicitacoes.append({
                "id_solicitacao":r[0],"tipo_solicitacao":r[1] or "nova_compra","status":r[2],
                "criado_em":str(r[3])[:19] if r[3] else "","obs_admin":r[4] or "",
                "aluno_nome":r[5],"aluno_email":r[6] or "",
                "id_pacote":r[7],"pacote_nome":r[8],"tipo_cobranca":r[9],
                "pacote_valor":float(r[10] or 0),"admin_nome":r[11] or "","modalidades":mods,
            })

        cursor.execute("""
            SELECT ap.id_aluno_pacote, ap.id_aluno, c.nome_completo, c.email,
                   p.nome, p.tipo_cobranca, p.valor, ap.data_inicio, ap.data_fim,
                   ap.status_acesso, ap.ultimo_pagamento, ap.proximo_vencimento
            FROM aluno_pacote ap
            JOIN cadastro c ON c.id_cadastro=ap.id_aluno
            JOIN pacotes p ON p.id_pacote=ap.id_pacote
            WHERE ap.ativo=1 ORDER BY ap.proximo_vencimento, c.nome_completo
        """)
        aluno_pacotes = []
        for r in cursor.fetchall():
            prox_venc = r[11]
            ...
            aluno_pacotes.append({
                "id_aluno_pacote":r[0],"id_aluno":r[1],"aluno_nome":r[2],"aluno_email":r[3] or "",
                "pacote_nome":r[4],"tipo_cobranca":r[5],"valor":float(r[6] or 0),
                "data_inicio":str(r[7]) if r[7] else "","data_fim":str(r[8]) if r[8] else "",
                "status_acesso":r[9] or "ativo","ultimo_pagamento":str(r[10]) if r[10] else "",
                "proximo_vencimento":str(prox_venc) if prox_venc else "","ciclo_dias":dias_ciclo(r[5]),
            })

        cursor.execute("""
            SELECT hpac.criado_em, c.nome_completo, p.nome, hpac.acao, hpac.observacao, adm.nome_completo
        ...
        """)
        historico = [{"criado_em":str(r[0])[:10] if r[0] else "","aluno_nome":r[1],
            "pacote_nome":r[2],"acao":r[3],"observacao":r[4] or "","admin_nome":r[5] or ""}
            for r in cursor.fetchall()]
        ...
        return jsonify(ok=True, solicitacoes=solicitacoes, aluno_pacotes=aluno_pacotes,
            historico=historico,
...
def pacotes_aprovar():
    ...
    try:
        ...
        """, (id_sol,))
        sol = cursor.fetchone()
        if not sol: return jsonify(ok=False, mensagem="Solicitação não encontrada ou já processada."), 404
        ...
        cursor.execute("""
            INSERT INTO aluno_pacote(id_aluno,id_pacote,data_inicio,status_acesso,ultimo_pagamento,proximo_vencimento,ativo)
            VALUES(?,?,?,'ativo',?,?,1)
        ...
        cursor.execute("""
            UPDATE solicitacoes_plano SET status='aprovada', processado_por=?, processado_em=GETDATE(), obs_admin=?
            WHERE id_solicitacao=?
        """, (id_admin, obs, id_sol))
        conn.commit()
        return jsonify(ok=True, mensagem="Solicitação aprovada e plano ativado!")
    except pyodbc.Error as e:
        if conn: conn.rollback()
        return jsonify(ok=False, mensagem=str(e)), 500
    finally:
...
def pacotes_negar():
    ...
    try:
        ...
        sol = cursor.fetchone()
        if not sol: return jsonify(ok=False, mensagem="Solicitação não encontrada."), 404
        cursor.execute("""
            UPDATE solicitacoes_plano SET status='negada', processado_por=?, processado_em=GETDATE(), obs_admin=?
            WHERE id_solicitacao=?
        """, (id_admin, motivo, id_sol))
        cursor.execute("""
            INSERT INTO historico_pacotes(id_aluno,id_pacote,acao,observacao,id_admin)
            VALUES(?,?,'negacao',?,?)
        """, (sol[0], sol[1], motivo or "Sem motivo", id_admin))
        conn.commit()
...
def registrar_pagamento():
    ...
    try:
        conn = get_conn(); cursor = conn.cursor()
        cursor.execute("""
            SELECT id_aluno, id_pacote, tipo_cobranca, valor FROM aluno_pacote ap
            JOIN pacotes p ON p.id_pacote=ap.id_pacote
            WHERE ap.id_aluno_pacote=? AND ap.ativo=1
        """, (id_aluno_pacote,))
        ap = cursor.fetchone()
        if not ap: return jsonify(ok=False, mensagem="Plano não encontrado."), 404
        id_aluno, id_pacote, tipo_cob, valor = ap
        d_pag = date.fromisoformat(data_pag)
        prox  = proximo_vencimento(d_pag, tipo_cob, d_pag)
        cursor.execute("""
            UPDATE aluno_pacote SET ultimo_pagamento=?, proximo_vencimento=?, status_acesso='ativo'
...
def suspender_aluno():
    ...
    try:
        ...
        conn.commit()
        return jsonify(ok=True, mensagem="Acesso suspenso.")
    except pyodbc.Error as e:
        if conn: conn.rollback()
        return jsonify(ok=False, mensagem=str(e)), 500
    finally:
...
def admin_aulas_experimentais():
    ...
    try:
        ...
        cursor.execute("""
            ...
            FROM aulas_experimentais ORDER BY criado_em DESC
        """)
        solicitacoes = []
        for r in cursor.fetchall():
            solicitacoes.append({
                "id":r[0],"nome":r[1],"telefone":r[2],"email":r[3] or "","cpf":r[4] or "",
                "modalidade":r[5],"horario_preferido":r[6] or "","mensagem":r[7] or "",
                "status":r[8] or "pendente","criado_em":str(r[9])[:16] if r[9] else "",
            })
        return render_template("admin_aulas_experimentais.html",
            user_name=session.get("user_name","Administrador"), solicitacoes=solicitacoes)
    except pyodbc.Error as e:
        flash(str(e),"erro")
        return render_template("admin_aulas_experimentais.html",
...
def atualizar_status_experimental(id_exp):
    ...
    try:
        conn = get_conn(); cursor = conn.cursor()
        cursor.execute("UPDATE aulas_experimentais SET status=?,observacao_admin=? WHERE id_experimental=?",
                       (status, obs, id_exp))
        conn.commit()
